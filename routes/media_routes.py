...
def upload_file():
    ...
    unique_filename = generate_unique_filename(original_file_name)

    # Save file and get Supabase path
    file_path = save_file(file, unique_filename)
    if not file_path:
        return jsonify({"error": "Invalid file type"}), 400

    # Save to DB
    media_entry = add_media(unique_filename, file_path)
    if not media_entry:
...
def fetch_all_media():
    ...
    return jsonify([{
        ...
        "media_img_id": os.path.join(Config.SUPABASE_STORAGE_URL, get_media_path(media.media_img_id, "image")) if media.media_img_id else None,
"media_vid_id": os.path.join(Config.SUPABASE_STORAGE_URL, get_media_path(media.media_vid_id, "video")) if media.media_vid_id else None,
"media_doc_id": os.path.join(Config.SUPABASE_STORAGE_URL, get_media_path(media.media_doc_id, "doc")) if media.media_doc_id else None,
